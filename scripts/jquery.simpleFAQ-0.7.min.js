...
h=setTimeout(function(){n.trigger('searchStart.simpleFAQ',[]);var sc=[];faqs.each(function(i){var f=$(this);var tg=f.find('.'+o.tagClass).text();tg=(o.ensitive)?tg:tg.toLowerCase();var t=f.text();t=(o.ensitive)?t:t.toLowerCase();var q=getQuery(sn.value,o);var s=0;if(q.length>0){s=$.score(t,q);s+=scoreTags(q,tg);}
...
return s;}
var getQuery=function(t,o){var q='';t=(o.ensitive)?t:t.toLowerCase();var ig=o.ignore;if(ig.length>0){var w=t.split(' ');for(var i=0;i<w.length;++i){if(w[i].length>0){if(typeof ig.indexOf=='function'){if(ig.indexOf(w[i])<0){q+=w[i]+' ';}}else{var f=false;for(var j=0;j<ig.length;++j){if(ig[j]==w[i]){f=true;break;}}
if(!f){q+=w[i]+' ';}}}}
...
if(typeof o.ensitive!='boolean'){o.ensitive=$.fn.simpleFAQ.defaults.ensitive;}
...
return o;}
$.fn.simpleFAQ.keyTimeout=400;$.fn.simpleFAQ.tagMatchScore=0.1;$.fn.simpleFAQ.defaults={data:null,nodeType:'li',questionClass:'question',answerClass:'answer',tagClass:'tags',showOnlyOne:false,allowSearch:false,searchNode:null,minSearchScore:0,sortSearch:false,ensitive:false,speed:300,ignore:['the','a','an','i','we','you','it','that','those','these','them','to','and','or','as','at','by','for','of','so']};})(jQuery);